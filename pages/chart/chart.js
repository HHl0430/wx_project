...
Page({
  ...
  btnclick() {
    wx.redirectTo({
      url: `${router.default.userChart}`
    })
  },
  projectDet(event) {
    wx.navigateTo({
      url: `${router.default.chartDetail}?years=${event.currentTarget.dataset['years']}`
    })
  },
  successData(res) {
    if (res.code == 200) {
  ...
  onLoad: function(options) {
    ...
    serves.getRequ(`itemsRate/statistics?beginTime=${this.data.beginTime}&endTime=${this.data.endTime}&userName=`, this.successData)
  },
